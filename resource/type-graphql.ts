...
export async function setup(extensionModule: string, options: ServerOptions): Promise<void> {
...
class DatabaseContainer implements ContainerType {
    ...
    get<T>(constructor: {
        new(tx: () => Promise<EntityManager>): T
    }): T {
        return new constructor(async () => {
            if (this.closed) {
                throw new Error('Response was already sent, too late to request transaction')
            }
            if (this.tx == null) {
    ...
    private createTx(): Promise<Tx> {
        return new Promise((resolve, reject) => {
            this.connection.transaction('SERIALIZABLE', em => {
